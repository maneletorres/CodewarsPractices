 ...
 *
 * '''Codewars exercise: "Salesman's Travel"'
 *
 * '''Description:''' A traveling salesman has to visit clients. He got each
 * client's address e.g.: "432 Main Long Road St. Louisville OH 43071" as a
 * list.
 *
 ...
 * format: zipcode:street and town,street and town,.../house number,house
 * number,...
 *
 ...
 * "zipcode:/"
 *
 * '''Examples:''' r = "123 Main Street St. Louisville OH 43071,432 Main Long
 ...
 * St. Louisville/123,432" travel(r, "NY 56432") --> "NY 56432:High Street
 * Pollocksville/786" travel(r, "NY 5643") --> "NY 5643:/"
 *
 */
public class Travel {

    public static void main(String[] args) {
        String ad = "123 Main Street St. Louisville OH 43071,"
                ...
                + "3 Main Rd. Bern AE 56210,77 Gordon St. Atlanta RE 13000";

        String code = "OH 43071,NY 56432,ZP 32908,AE 56210,RE 13000,EX 34342,"
                + "SW 43098,AA 45521,GG 30654,ZP 32908,AE 56215,RE 13200,"
                + "EX 34345,RE 13222,RE 13001,SW 43198,AA 45522,GG 30655,"
                + "XX 32321,YY 45098";

        String[] codeSplitted = code.split(",");

        System.out.println("The result of searching for the addresses that "
                + "match the postal code '" + codeSplitted[16] + "' is: "
                + travel(ad, codeSplitted[16]));
    }

    public static String travel(String r, String zipcode) {
        String[] addressesSplitted = r.split(",");
        String[] zipcodeSplitted = zipcode.split(" ");

        String address = "";
        String addressNumber = "";
        for (String aux : addressesSplitted) {
            String[] internalSplit = aux.split(" ");
            if (zipcodeSplitted[0].equals(internalSplit[internalSplit.length - 2]) && zipcodeSplitted[1].equals(internalSplit[internalSplit.length - 1])) {
                if (address.length() > 0) {
                    address += ",";
                }
                ...
                for (int j = 0; j < internalSplit.length; j++) {
                    if (j == 0) {
                        addressNumber += internalSplit[j];
                    } else if (j < internalSplit.length - 2) {
                        if (j != internalSplit.length - 3) {
                            address += internalSplit[j] + " ";
                        } else {
                            address += internalSplit[j];
                        }
                    }
        ...
        return address.length() == 0 ? zipcode + ":/" : zipcode + ":" + address + "/" + addressNumber;
    }
}
